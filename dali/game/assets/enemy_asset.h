...
struct EnemyAsset {
    ...
    // This struct IS the spawn-snapshot boundary: a field here is snapshotted; anything on
    // EnemyAsset outside it (references, resolved pointers) is not. Applying a blueprint is a
    // single memberwise copy (enemy.Data = asset.Data), so a new stat is added in exactly one place
