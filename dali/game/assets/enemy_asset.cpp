...
// sub-map: compact and the usual way a color is authored. Byte order is written out explicitly so it
// does not depend on Color32's union endianness.
i32 HexDigit(char c) {
    if (c >= '0' && c <= '9') {
        return c - '0';
    }
...
// and returns false unless the string is exactly 8 hex digits.
...
std::optional<EnemyAsset> EnemyAsset::LoadFromDisk(AssetId id) {
    ...
    asset.Manifest.Id = id;
    asset.Manifest.HasPayload = false;
    asset.Data = ParseInstanceData(node);
    return asset;
}
